...
<form action= "<?php echo $tab?>.php" method='POST' class='input-group mb-3' style='z-index: 0; margin-top: 6px; margin-right: 30px;'>
	...
	<div class="vertical form-control">
			<input type="search" name="search" id="search" class="form-control" placeholder="Search for J.O. #" onkeyup="doSearch()"  aria-label="search for J.O. #" aria-describedby="basic-addon2">
		<h6>Filter:</h6>
		...
		<!-- <label>Artist: </label>
		 ...
		 <label>Agent: </label>
		  <select name="agent" onkeyup="searchAgent()" id="agent">
		  	<option selected="true" disabled="disabled">-Select-</option>
		  	<option  value="" >None</option>
		    <?php
		...
		<?php 
			...
			if($tab!="reports_this_year"||$tab!="reports_last_year") {
			?>
			 <label>Month: </label>
			  <select name="month">
			  	<option selected="true" disabled="disabled" >-Select-</option>
 ...
 ?> -->
		 ...
		 <label>View Report on date From:</label> 
             <input type="date" name="dateStart"  id="dateStart" value="<?php if(isset($_SESSION['dateStart'])){echo $_SESSION['dateStart'];}?>">&nbsp;<label> date To:</label> 
              <input type="date" name="dateEnd"  id="dateEnd" value="<?php if(isset($_SESSION['dateEnd'])){echo $_SESSION['dateEnd'];}?>"> &nbsp; <input type="submit" name="go" id="go" value="Go">
