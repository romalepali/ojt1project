...
<body>
	...
	// var total_amount = total.toLocaleString(undefined, {minimumFractionDigits: 2});
	// //wala pay date ang print idk unsaon pa lmao
 //    var htmlToPrint = '' + '<style type="text/css">'+ 'table th{' +'border:1px solid black;' +'padding:0.5em;'+ 'text-align:center;' +'}'+'table td {' + 'border:1px solid #aaa;' + 'padding:0.5em;' + '}' +'</style>';
 //    var signingPeople = '' + '<style type="text/css">'  +''+ '</style>'

    htmlToPrint += divToPrint.outerHTML;
    // signingPeople += tableToPrint.outerHTML;
        newWin = window.open("");
    newWin.document.write("<h2 align='center' style='font-weight:bold;'>IMAGEWORLD DIGITAL PRINTING, INC.</h2>"+ "<h3 align='center' style='font-weight:bold;'>MONTHLY INVENTORY REPORT </h3>"+htmlToPrint +"<h2 style ='float:right;margin-right:15px;'>" +"GRAND TOTAL: " +'₱ ' + total_amount +"</h2>" + '<br>'+ '<br>'+ '<br>'+ signingPeople);

    newWin.print();
    newWin.close();
}
</script>
<button type="button" class="btn btn-default btn-sm" style="float: right; position: relative; background-color: #0064b0; color: #fff; margin-right: 10px" onclick="print()"><span class="glyphicon glyphicon-print"></span> Print</button>
</body>
</html>